...
def print_generation(population):
    ...
    table_data = prettytable.PrettyTable(['schedule number', 'fitness', 'numb of conflicts', 'classes [spec,class,audience,teacher,lesson time]'])  #, 'classes [spec,class,audience,teacher,lesson time]'])
    for i in range(0, len(schedules)):
        table_data.add_row([str(i), str(round(schedules[i]._fitness,3)), str(schedules[i]._problems), schedules[i].__str__()])  #, schedules[i].__str__()])
    print(table_data)
...
population._schedules.sort(key=lambda x: x.get_fitness(), reverse=True)
print_generation(population)
print_schedule_as_table(population._schedules[0])
geneticAlgorithm = GeneticAlgorithm()
while (population._schedules[0].get_fitness() != 1.0):
    generationNumber += 1
    print("\n> Generation # " + str(generationNumber))
